...
void red_solowp() {
	...
	moveToPoint({28, 48}, fwd, 80);
	pidWait(Wait::WAIT);
	moveToPoint({72, 27}, fwd, 80);
	pidWait(Wait::QUICK);
	setMogo(false);
	// grab mogo
...
void red_7greed() {
	...
	if(ladderBool) {
		...
		driveSet(40, 127);
		pidWait(Wait::CHAIN);
		swingSet(RIGHT_SWING, 0, 90, 20, ccw);
	}
	pidWait(Wait::WAIT);
}
...
void blue_7greed() {
	...
	if(ladderBool) {
		...
		driveSet(10, 127);
		pidWait(Wait::CHAIN);
		swingSet(LEFT_SWING, 0, 90, 20, cw);
	}
	pidWait(Wait::WAIT);
}
